...
def process_file(filename, block_size=1024):
    ...
    for block in blocks:
        ...
        encoded_blocks.append(mtf_encoded)
        block_lengths.append(len(mtf_encoded)) 

    combined_data = b''.join(encoded_blocks)
    encoded_bytes, codebook, padding = encode_huffman(combined_data)
    compressed_filename = 'HelpPane_bmh_enc.exe'
...
#if b'\x00' not in mtf_decoded:
#            mtf_decoded += b'\x00'
#with open("Kuprin.txt", "rb") as f_in:
#    block_number = 0
#    while True:
...
#        bwt_encoded = bwt_encode(block)
#        mtf_encoded = mtf_encode(bwt_encoded)
#        encoded_bytes, codebook, padding = encode_huffman(mtf_encoded)
#        write_compressed_file('Kuprin_bmh_enc.txt', encoded_bytes, codebook, padding)
#        block_number += 1
#        print('1')

#with open('Kuprin_bmh_dec.txt', 'wb') as f_out:
#    while True:
#        encoded_bytes, codebook, padding = read_compressed_file('Kuprin_bmh_enc.txt')
#        if not encoded_bytes:
#            break
#        decoded_data = decode_huffman(encoded_bytes, codebook, padding)
#        mtf_decoded = mtf_decode(decoded_data)
#        bwt_decoded = bwt_decode(mtf_decoded)
#        f_out.write(bwt_decoded)
#        print('0')

#with open("Kuprin.txt", "rb") as f_in, open('Kuprin_bmh_dec.txt', 'rb') as f_out:
